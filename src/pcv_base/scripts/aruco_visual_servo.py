...
class arucoVisualServoing:
  ...
  def getTransformFromTwoCoords(self):
        ...
        center = (self.relPosL + self.relPosR)*0.5
        # angle of the feature, i.e. the angle of the target surface with the camera plane
        angle = np.arctan2(self.relPosR[0]-self.relPosL[0], -self.relPosR[1]+self.relPosL[1])
        ...
        while not rospy.is_shutdown():
            ...
            if len(corners) > 0:
                ...
                for (markerCorner, markerID) in zip(corners, ids):
                    if markerID == self.refL:
                        (topLeft, topRight, bottomRight, bottomLeft) = markerCorner[0]
                        coordL = np.array([topLeft[0]+topRight[0]+bottomRight[0]+bottomLeft[0], \
                                           topLeft[1]+topRight[1]+bottomRight[1]+bottomLeft[1]])*0.25
                        self.relPosL = self.getRelativePos(coordL)
                        Ldetected = True
                    elif markerID == self.refR:
                        (topLeft, topRight, bottomRight, bottomLeft) = markerCorner[0]
                        coordR = np.array([topLeft[0]+topRight[0]+bottomRight[0]+bottomLeft[0], \
                                           topLeft[1]+topRight[1]+bottomRight[1]+bottomLeft[1]])*0.25
                        self.relPosR = self.getRelativePos(coordR)
                        Rdetected = True
                # both codes detected
                #print(Ldetected, Rdetected)
                if Ldetected and Rdetected:
                    self.getTransformFromTwoCoords()
                    self.control()
                else:
            ...
            #    self.control()
                if abs(self.errX) <= 0.01 and abs(self.errY)<=0.01 and abs(self.errAng)<=0.01 and self.controlValid:
                    print(self.errX, self.errY, self.errAng)
                    self.vel_pub.publish(Twist())
