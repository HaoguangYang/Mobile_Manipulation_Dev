...
def joystickTeleopBase()
    class joystickTeleopBaseClass(RobotBase):
        def __init__(self):
            super(joystickTeleopBaseClass, self).__init__()
        ...
        def execute(self):
            super(joystickTeleopBaseClass, self).execute()
        ...
        def shutdown(self):
            super(joystickTeleopBaseClass, self).shutdown()
            self.__joystick_inst.stop()
            self.__joy_if_inst.shutdown()
            self.robot_enable(0)
            
        def evaluate(self):
            super(joystickTeleopBaseClass, self).evaluate()
            
        def estimate(self):
            super(joystickTeleopBaseClass, self).estimate()
