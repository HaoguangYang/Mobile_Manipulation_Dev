...
class Task():
    def __init__(self, uuid, location, init_pos = 1):
        ...
        self.rcfgClient = dynamic_reconfigure.client.Client("move_base")
    ...
    def evaluate(self):
        ...
        elif self.targetActionList[self.waypt_idx][1] == 'vel':
            params = { 'base_local_planner/max_vel_x' : float(self.targetActionList[self.waypt_idx][2]) }
            #           'base_local_planner/weight_optimaltime': 300.0}      # need to test
            self.rcfgClient.update_configuration(params)
        elif self.targetActionList[self.waypt_idx][1] == 'avel':
            params = { 'base_local_planner/max_vel_theta' : float(self.targetActionList[self.waypt_idx][2]) }
            self.rcfgClient.update_configuration(params)
        else:
